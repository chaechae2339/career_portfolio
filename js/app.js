  ...
  const projects = [
    ...
    },
    {
      ...
      outcomeSteps: [
        ...
        { title: '모니터링 체계로서의 정착', desc: '재무 영향별/공정별/ESG 영역별로 협력사 간 차이와 평균 등을 포함한 엑셀 기반 대쉬보드를 구축하여 롯데건설 공급망 모니터링 체계를 구축했습니다. 이를 통해 공급망에서 발생할 수 있는 리스크를 사전에 식별하는 근거 자료 및 경영진 보고 자료로 활용하면서 매년 평가를 통해 업데이트할 수 있는 내부 모니터링 체계로 정착시켰습니다.', image: 'projects/images/images:lotteenc1.png', imageAlt: '롯데건설 파트너사 ESG 자가진단 기준 및 체크리스트, 실시 프로세스, 2025년 자가진단 등급 분포 및 주요 리스크 요인 대시보드' },
      ],
    },
    {
      ...
      outcomeSteps: [
        { title: '2년 만의 자체 ESG 관리 체계 구축', desc: '전사 거버넌스와 데이터 관리 체계를 갖춰, 기업 스스로 ESG를 점검하고 실행할 수 있는 체계를 완성했습니다.' }, image: 'projects/images/images:shiwon1.png', imageAlt: '신원 ESG Vision House — ESG 전략 체계, ESG Roadmap, 신원 ESG 전략 체계 및 거버넌스 조직도' },
    ...
    const stepsHtml = (steps, barColor) => steps.map((s) => `
      <div class="pg-step">
        ...
        <div>
          ...
          ${s.image ? `<figure class="pg-step-figure"><img src="${esc(s.image)}" alt="${esc(s.imageAlt || s.title)}"></figure>` : ''}
        </div>
      </div>`).join('');
